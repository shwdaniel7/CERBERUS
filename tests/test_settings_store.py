...
def test_missing_file_returns_defaults(store):
    ...
    for name in ("blacklist", "virustotal", "strings", "ioc_extract", "entropy", "magic_numbers", "pe_analysis", "deobfuscation", "yara"):
...
def test_unknown_keys_are_dropped_on_save(store):
    ...
    assert json.loads(store.read_text(encoding="utf-8")) == {  # no unknown key leaks to disk
        ...
        "entropy": True, "magic_numbers": True, "pe_analysis": True, "deobfuscation": True, "yara": True,
